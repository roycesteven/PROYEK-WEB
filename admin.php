            ...
            <?php
                ...
                if(isset($_POST['search'])){
                    ...
                    try{
                        if($sort == "ascending"){
                            if($attr == "username"){
                                $stmt=$pdo->prepare("select * from user where username like :nama ORDER BY username ASC");
                            }else if($attr == "id"){
                                $stmt=$pdo->prepare("select * from user where id like :nama ORDER BY id ASC");
                            }else if($attr == "nomor_telp"){
                                $stmt=$pdo->prepare("select * from user where nomor_telp like :nama ORDER BY nomor_telp ASC");
                            }else if($attr == "alamat"){
                                $stmt=$pdo->prepare("select * from user where alamat like :nama ORDER BY alamat ASC");
                            }
                        }else if($sort == "descending"){
                            if($attr == "username"){
                                $stmt=$pdo->prepare("select * from user where username like :nama ORDER BY username DESC");
                            }else if($attr == "id"){
                                $stmt=$pdo->prepare("select * from user where id like :nama ORDER BY id DESC");
                            }else if($attr == "nomor_telp"){
                                $stmt=$pdo->prepare("select * from user where nomor_telp like :nama ORDER BY nomor_telp DESC");
                            }else if($attr == "alamat"){
                                $stmt=$pdo->prepare("select * from user where alamat like :nama ORDER BY alamat DESC");
                            }
                        }else{
                            if($attr == "username"){
                                $stmt=$pdo->prepare("select * from user where username like :nama");
                            }else if($attr == "id"){
                                $stmt=$pdo->prepare("select * from user where id like :nama");
                            }else if($attr == "nomor_telp"){
                                $stmt=$pdo->prepare("select * from user where nomor_telp like :nama");
                            }else if($attr == "alamat"){
                                $stmt=$pdo->prepare("select * from user where alamat like :nama");
                            }
                        }
